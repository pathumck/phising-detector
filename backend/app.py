...
def reload_blacklist_route():
    """
    Forces a full reload of the in-memory BLACKLIST set from the
    database. Use this after making direct database edits (e.g. via
    SQLTools/sqlite3 CLI) to sync the running server without a restart.
    """
...
def record_override():
    """
    Records that a user dismissed a warning and proceeded to a URL
    anyway. Called by the extension's "proceed anyway" button.
    Body: {"url": "...", "tab_id": "..."}  (tab_id optional)
    """
    data = request.get_json(silent=True)
    if not data or "url" not in data:
...
def check_override():
    """
    Returns whether this URL has already been overridden recently,
    so the extension can skip re-warning on a page the user already
    chose to proceed through this session.
...
def stats():
    """
    Aggregated verdict_log + user_overrides stats for the admin
    dashboard and project evaluation chapter: per-layer trigger
    counts, verdict split, average latency, and domains that keep
    getting overridden (candidate false positives).
    """
